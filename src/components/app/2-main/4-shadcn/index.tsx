...
function ColorPreview({ color }: { color?: string; }) {
    return (<>
        {color
            ?
            <div className="flex-none aspect-square w-9 h-9 ring-border ring-2 rounded overflow-hidden" style={{ backgroundColor: `hsl(${color})` }}></div>
            :
            <IconNoColor className="aspect-square w-9 h-9 stroke-none bg-neutral-100 fill-neutral-400 ring-border ring-2 rounded overflow-hidden" />
        }
    </>);
}
